...
# File Name : monetarydonation_statement.py
...
class PledgeStatement:
    ...
    def pledge_byduration_statement_window(self):
        pledge_statement_window = Toplevel(self.master)

        pledge_statement_window.title("Pledge Statement ")
        pledge_statement_window.geometry('770x600+120+40')
        pledge_statement_window.configure(background='wheat')
        pledge_statement_window.resizable(width=False, height=False)

        # delete "X" button in window will be not-operational
        pledge_statement_window.protocol('WM_DELETE_WINDOW', self.obj_commonUtil.donothing)
        imageFrame = Frame(pledge_statement_window, width=65, height=60,
                           bg="wheat")
        ...
        infoFrame = Frame(pledge_statement_window, width=300, height=100, bd=6, relief='ridge',
                          bg="wheat")
        topFrame = Frame(pledge_statement_window, width=300, height=100, bd=6, relief='ridge',
                         bg="light yellow")
        ...
        middleFrame_bookdisplay = Frame(pledge_statement_window, width=0, height=0, bd=8, relief='ridge')

        heading = Label(pledge_statement_window, text="Sankalp-Seva Statement",
        ...
        category_label = Label(category_frame, text="Select Pledge Item", width=14, justify='center', font=NORM_FONT,
                               bg='light yellow', state=NORMAL)
        category_label.grid(row=1, column=0, padx=10, pady=10)
        trust_label = Label(category_frame, text="Select Trust", width=14, justify='left', anchor='w', font=NORM_FONT,
                            bg='light yellow', state=NORMAL)
        trust_label.grid(row=2, column=0, padx=10, pady=10)

        categoryText = StringVar(category_frame)
        category_list = self.obj_commonUtil.getPledgeItemNames()
        category_list.append("All")
        categoryText.set(category_list[0])
        ...
        trustText = StringVar(category_frame)
        trust_list = ['Aadarsh gaushala Trust', 'Vihangam Yoga (Karnataka) Trust']
        trustText.set(trust_list[0])
        trust_menu = OptionMenu(category_frame, trustText, *trust_list)
        trust_menu.configure(font=NORM_FONT, width=18, anchor=W, justify='left', bg='snow')
        trust_menu.grid(row=2, column=1, padx=10, pady=10)
        ...
        printBtn = Button(buttonFrame, text="Print Statement", fg="Black",
                          font=NORM_FONT, width=12, bg='light grey', state=DISABLED)
        cancel = Button(buttonFrame, text="Close", fg="Black", command=pledge_statement_window.destroy,
                        font=NORM_FONT, width=12, bg='light cyan')
        search_result = partial(self.prepare_account_statement_Excel, pledge_statement_window, printBtn,
                                cal_dateFrom,
                                cal_toDate, month_variable, year_variable,
                                year_yearvariable, var, categoryText, trustText, viewPDF, infoLabel, cancel)
        ...
        dateFrame.grid(row=4, column=0, padx=10, pady=10)
        fromDate.grid(row=1, column=0, padx=10)
        cal_dateFrom.grid(row=1, column=1)
        toDate.grid(row=1, column=2, padx=10)
        cal_toDate.grid(row=1, column=3)

        # create a Close Button and place into the pledge_statement_window window

        cancel.grid(row=0, column=3)
        # ---------------------------------Button Frame End----------------------------------------
        middleFrame_bookdisplay.grid(row=2, column=2, padx=65, pady=10, sticky=W)

        pledge_statement_window.bind('<Return>', lambda event=None: submit.invoke())
        pledge_statement_window.bind('<Alt-c>', lambda event=None: cancel.invoke())

        pledge_statement_window.focus()
        pledge_statement_window.grab_set()
        mainloop()

    def pledge_byItem_statement_window(self):
        pledge_statement_window = Toplevel(self.master)

        pledge_statement_window.title("Pledge Statement ")
        pledge_statement_window.geometry('810x470+120+40')
        pledge_statement_window.configure(background='wheat')
        pledge_statement_window.resizable(width=False, height=False)
        ...
        topFrame = Frame(pledge_statement_window, width=300, height=100, bd=6, relief='ridge',
                         bg="light yellow")
        topFrame.grid(row=2, column=0, padx=90, pady=10, sticky=W)
        upperFrame = Frame(topFrame, width=300, height=100, bg="light yellow")
        upperFrame.grid(row=1, column=0, padx=10, pady=10, sticky=W)
        infoFrame.grid(row=3, column=0, padx=100, pady=10, sticky=W)
        infoLabel = Label(infoFrame, text="Select appropriate parameters and press Search", width=65,
                          justify='center', font=('arial narrow', 13, 'bold'),
                          bg='snow', fg='green', state=NORMAL)
        infoLabel.grid(row=0, column=0)
        ...
        heading.grid(row=1, column=0)
        category_frame = Frame(upperFrame, width=100, height=100, bd=0, relief='ridge', bg='light yellow')
        category_frame.grid(row=1, column=0, padx=20, pady=10)

        trust_label = Label(category_frame, text="Select Trust", width=14, justify='left', anchor='w',
                            font=NORM_FONT_MEDIUM_HIGH,
                            bg='light yellow', state=NORMAL)
        trust_label.grid(row=1, column=0, padx=10, pady=10)

        category_label = Label(category_frame, text="Select Pledge Item", width=14, justify='center',
                               font=NORM_FONT_MEDIUM_HIGH,
                               bg='light yellow', state=NORMAL)
        category_label.grid(row=2, column=0, padx=10, pady=10)

        status_label = Label(category_frame, text="Select Status", width=14, justify='left', anchor='w',
                             font=NORM_FONT_MEDIUM_HIGH,
                             bg='light yellow', state=NORMAL)
        ...
        trustText.set(trust_list[0])
        trust_menu = OptionMenu(category_frame, trustText, *trust_list)
        trust_menu.configure(font=NORM_FONT_MEDIUM_HIGH, width=25, anchor=W, justify='left', bg='snow')
        trust_menu.grid(row=1, column=1, padx=10, pady=10)
        ...
        categoryText.set(category_list[0])
        category_menu = OptionMenu(category_frame, categoryText, *category_list)
        category_menu.configure(font=NORM_FONT_MEDIUM_HIGH, width=25, anchor=W, justify='left', bg='snow')
        category_menu.grid(row=2, column=1, padx=10, pady=10)

        trustStatusText = StringVar(category_frame)
        trustStatus_list = ['Closed', 'Open']
        trustStatusText.set(trustStatus_list[0])
        trustStatus_menu = OptionMenu(category_frame, trustStatusText, *trustStatus_list)
        trustStatus_menu.configure(font=NORM_FONT_MEDIUM_HIGH, width=25, anchor=W, justify='left', bg='snow')
        trustStatus_menu.grid(row=3, column=1, padx=10, pady=10)
        ...
        viewPDF = Button(buttonFrame, text="View Statement", fg="Black",
                         font=NORM_FONT_MEDIUM_LOW, width=12, bg='light grey', state=DISABLED)

        printBtn = Button(buttonFrame, text="Print Statement", fg="Black",
                          font=NORM_FONT_MEDIUM_LOW, width=12, bg='light grey', state=DISABLED)
        cancel = Button(buttonFrame, text="Close", fg="Black", command=pledge_statement_window.destroy,
                        font=NORM_FONT_MEDIUM_LOW, width=12, bg='light cyan')
        search_result = partial(self.prepare_account_statement_byitem, pledge_statement_window, printBtn,
                                categoryText, trustText, trustStatusText, viewPDF, infoLabel, cancel)
        submit = Button(buttonFrame, text="Search", fg="Black", command=search_result,
                        font=NORM_FONT_MEDIUM_LOW, width=12, bg='light cyan')
        submit.grid(row=0, column=0)
        viewPDF.grid(row=0, column=1)
    ...
    def pledge_byMember_statement_window(self):
        pledge_statement_window = Toplevel(self.master)

        pledge_statement_window.title("Member Pledge Statement ")
        pledge_statement_window.geometry('810x520+120+40')
        pledge_statement_window.configure(background='wheat')
        pledge_statement_window.resizable(width=False, height=False)
        ...
        heading = Label(pledge_statement_window, text="Member Pledge Statement",
                        font=('times new roman', 20, 'bold'),
                        bg="wheat")
        ...
        memberId_label = Label(category_frame, text="Member Id", width=14, justify='left', anchor='w',
                               font=NORM_FONT_MEDIUM_HIGH,
                               bg='light yellow', state=NORMAL)
        memberId_label.grid(row=1, column=0, padx=10, pady=10)
        ...
        memberId_Text = Entry(category_frame, width=29, font=NORM_FONT_MEDIUM_HIGH, bd=2, relief='ridge',
                              bg='light yellow')
        memberId_Text.grid(row=1, column=1, padx=5)
        ...
        category_menu = OptionMenu(category_frame, categoryText, *category_list)
        category_menu.configure(font=NORM_FONT_MEDIUM_HIGH, width=25, anchor=W, justify='left', bg='snow')
        category_menu.grid(row=3, column=1, padx=10, pady=10)
        ...
        trustStatus_menu = OptionMenu(category_frame, trustStatusText, *trustStatus_list)
        trustStatus_menu.configure(font=NORM_FONT_MEDIUM_HIGH, width=25, anchor=W, justify='left', bg='snow')
        trustStatus_menu.grid(row=4, column=1, padx=10, pady=10)

        # ---------------------------------Button Frame Start----------------------------------------
        buttonFrame = Frame(category_frame, width=100, height=100, bd=4, relief='ridge', bg="light yellow")
        buttonFrame.grid(row=5, column=0, padx=10, pady=5, columnspan=3)
        ...
        cancel = Button(buttonFrame, text="Close", fg="Black", command=pledge_statement_window.destroy,
                        font=NORM_FONT_MEDIUM_LOW, width=12, bg='light cyan')
        search_result = partial(self.prepare_account_statement_byMember, pledge_statement_window, printBtn,
                                categoryText, trustText, memberId_Text, trustStatusText, viewPDF, infoLabel, cancel)
        submit = Button(buttonFrame, text="Search", fg="Black", command=search_result,
                        font=NORM_FONT_MEDIUM_LOW, width=12, bg='light cyan')
        submit.grid(row=0, column=0)
        viewPDF.grid(row=0, column=1)
    ...
    def closepage_statmentbypledgeItem(self, src_file, starting_index, account_statement_window):
        print("closepage_statmentbypledgeItem :", src_file)
        account_statement_window.destroy()
        # erase the written records in template sheet
        # executed only if
        wb_template = openpyxl.load_workbook(src_file)
        ...
        for rows in range(5, starting_index + 1):
    ...
    def prepare_account_statement_Excel(self, account_statement_window, printBtn, cal_dateFrom,
                                        ...
                                        viewbyYear_yearTxt, var, categoryText, trustText, viewPDF, infoLabel,
                                        cancelbtn):
        ...
        if bDateConditionsValid:
            if from_year == self.obj_commonUtil.getCurrentYearFolderName() and \
                ...
                path_seva_sheet = InitDatabase.getInstance().initilize_current_year_pledge_payment_database(
                    trustText.get(), categoryText.get())
                # open current year pledge item sheet and write the data --start
                sorted_seva_sheet = InitDatabase.getInstance().initilize_sorted_current_year_pledge_payment_database()
                if bAlike_seva:
                    ...
                    if total_records > 0:
                        ...
                        for row_index in range(0, total_records):
                            ...
                            dateFromTransactionSheet = self.dateTimeOp.prepare_dateFromString(
                                sheet_obj.cell(row=row_index + 2, column=5).value)
                            # print("dateFromMon_DepositSheet :", dateFromTransactionSheet, "fromDate :", fromDate, " toDate:", toDate)
                            print("Selected Pledge item filter is :", categoryText.get())
                            if categoryText.get() == "All":
                                ...
                                if ((dateFromTransactionSheet > fromDate or dateFromTransactionSheet == fromDate)
                                        and (dateFromTransactionSheet < toDate or dateFromTransactionSheet == toDate)
                                        and (sheet_obj.cell(row=row_index + 2, column=6).value == categoryText.get())):
                        ...
                        destination_file = "..\\Expanse_Data\\" + currentyear + "\\Seva_Rashi\\Statements\\Pledge_Donation_" + dt_string + ".pdf"
                        # write the  sorted record in statement template
                        if trustText.get() == "Aadarsh gaushala Trust":
                            template_statement = "..\\Expanse_Data\\" + currentyear + "\\Seva_Rashi\\Template\\pledge_statement_template_gaushala.xlsx"
                        else:
                            template_statement = "..\\Expanse_Data\\" + currentyear + "\\Seva_Rashi\\Template\\pledge_statement_template.xlsx"
                        ...
                        if total_sorted_records > 0:
                            ...
                            for row_index in range(1, total_sorted_records + 1):
                                ...
                                for column_index in range(1, 6):
                                    if column_index == 1:  # Date
                                        text_value = sort_sheet.cell(row=row_index + 1, column=5).value
                                        ...
                                        text_value = sort_sheet.cell(row=row_index + 1, column=6).value + "-By " + \
                                                     sort_sheet.cell(row=row_index + 1, column=10).value + \
                                                     " From " + sort_sheet.cell(row=row_index + 1, column=4).value

                                    elif column_index == 4:  # credit amount
                            ...
                            obj_threadClass = myThread(10, "statementThread", 1, template_statement,
                ...
                for year_loop in range(1, loop_range):
                    ...
                    if total_records > 0:
                        ...
                        if total_sorted_records > 0:
                            ...
                            print("File has been saved for template")
                            destination_copy_folder = InitDatabase.getInstance().get_desktop_statement_directory_path()
                            obj_threadClass = myThread(15, "pledgestatementThread", 1, template_sheet,
                                                       destination_file, starting_index, viewPDF, printBtn, infoLabel,
                                                       destination_copy_folder)
                            obj_threadClass.start()
    ...
    def prepare_account_statement_byitem(self, account_statement_window, printBtn,
                                         categoryText, trustText, trustStatusText, viewPDF, infoLabel, cancel):
        print("prepare_account_statement_Excel ")

        viewPDF.configure(state=DISABLED, bg="light grey")
        printBtn.configure(state=DISABLED, bg="light grey")

        # open pledge item sheet and write the data --start
        if categoryText.get() == "All":
            pledge_file_path = InitDatabase.getInstance().initilize_pledgeitem_database(trustText,
                                                                                        "Pledge_Master")
        else:
            pledge_file_path = InitDatabase.getInstance().initilize_pledgeitem_database(trustText, categoryText.get())
        sorted_seva_sheet = InitDatabase.getInstance().initilize_sorted_pledgeitem_database(trustText,
                                                                                            categoryText.get())
        print("path_seva_sheet :", pledge_file_path, "sorted_seva_sheet :", sorted_seva_sheet)
        wb_obj = openpyxl.load_workbook(pledge_file_path)
        sheet_obj = wb_obj.active
        wb_sorted_obj = openpyxl.load_workbook(sorted_seva_sheet)
        sheet_sorted_obj = wb_sorted_obj.active
        total_records = self.obj_commonUtil.totalrecords_excelDataBase(pledge_file_path)
        if total_records > 0:
            sort_sheet_index = 2
            print("Total records  in [pledge] sheet:", total_records)
            for row_index in range(0, total_records):
                # print("Date from sheet is :", sheet_obj.cell(row=row_index + 2, column=6).value)
                # print("dateFromMon_DepositSheet :", dateFromTransactionSheet, "fromDate :", fromDate, " toDate:", toDate)
                print("Selected Pledge item filter is :", categoryText.get())
                if sheet_obj.cell(row=row_index + 2, column=13).value == trustStatusText.get():
            ...
            dt_string = now.strftime("%d_%b_%Y_%H%M%S")
            currentyear = now.strftime("%Y")
            destination_file = "..\\Expanse_Data\\" + currentyear + "\\Seva_Rashi\\Statements\\" + categoryText.get() + "_" + trustStatusText.get() + dt_string + ".pdf"
            # write the  sorted record in statement template
            if trustText.get() == "Aadarsh gaushala Trust":
                template_statement = "..\\Expanse_Data\\" + currentyear + "\\Seva_Rashi\\Template\\Pledge_Item_template_gaushala.xlsx"
            else:
                template_statement = "..\\Expanse_Data\\" + currentyear + "\\Seva_Rashi\\Template\\Pledge_Item_template.xlsx"
            ...
            if total_sorted_records > 0:
                ...
                for row_index in range(1, total_sorted_records + 1):
                    ...
                    for column_index in range(1, 9):
                        if column_index == 1:
                            text_value = dict_index
                        elif column_index == 2:  # Name
                            text_value = sort_sheet.cell(row=row_index + 1, column=3).value
                        elif column_index == 3:  # Date
                            text_value = sort_sheet.cell(row=row_index + 1, column=7).value
                            text_value = text_value.strftime("%d-%b-%Y")
                        elif column_index == 4:  # Amt
                            text_value = sort_sheet.cell(row=row_index + 1, column=5).value
                        elif column_index == 5:  # contact number
                            text_value = str(sort_sheet.cell(row=row_index + 1, column=4).value)
                        elif column_index == 6:  # duration
                            text_value = sort_sheet.cell(row=row_index + 1, column=10).value
                        elif column_index == 7:  # rem balance
                            text_value = sort_sheet.cell(row=row_index + 1, column=12).value
                        elif column_index == 8:  # rem balance
                            text_value = sort_sheet.cell(row=row_index + 1, column=13).value
                        else:
                            pass
                        critical_stock_sheet.cell(row=starting_index, column=column_index).font = Font(
                            size=14,
                            ...
                            bold=False)
                        if column_index == 5 or column_index == 6 or column_index == 4 or \
                                column_index == 7 or column_index == 8 or \
                                column_index == 1 or column_index == 3:
                            critical_stock_sheet.cell(row=starting_index,
                                                      column=column_index).alignment = Alignment(
                                horizontal='center', vertical='center', wrapText=True)
                        else:
                ...
                critical_stock_sheet.cell(row=2, column=8).value = dt_today
                critical_stock_sheet.cell(row=3, column=2).value = trustStatusText.get()
                critical_stock_sheet.cell(row=2, column=4).value = categoryText.get()
                critical_stock_sheet.cell(row=3, column=8).value = str(starting_index - 5)

                wb_critical_stock.save(template_statement)
                print("File has been saved for template")
                destination_copy_folder = InitDatabase.getInstance().get_desktop_statement_directory_path()
                obj_threadClass = myThread(12, "stockinfoThread", 1, template_statement,
                                           destination_file, starting_index, viewPDF, printBtn, infoLabel,
                                           destination_copy_folder)
                obj_threadClass.start()
                ...
                print_result = partial(self.obj_commonUtil.print_statement_file,
                                       ...
                                       destination_file, starting_index)
                printBtn.configure(command=print_result)
                view_result = partial(self.viewPDF, template_statement,
                                      destination_file, starting_index)
                viewPDF.configure(command=view_result)

                cancel_result = partial(self.closepage_statmentbypledgeItem, template_statement, starting_index,
                                        account_statement_window)
                cancel.configure(command=cancel_result)
                result_categoryChangeState = partial(self.checkCategoryChange, template_statement,
                                                     starting_index,
                                                     "None")
                categoryText.trace("w", result_categoryChangeState)
    ...
    def prepare_account_statement_byMember(self, account_statement_window, printBtn,
                                           categoryText, trustText, memberId_Text, trustStatusText, viewPDF, infoLabel,
                                           cancel):
        print("prepare_account_statement_Excel ")

        viewPDF.configure(state=DISABLED, bg="light grey")
        printBtn.configure(state=DISABLED, bg="light grey")
        bMemberExists = self.obj_commonUtil.validate_memberId_Excel(memberId_Text.get(), 1)
        if bMemberExists:
            # open pledge item sheet and write the data --start
            if categoryText.get() == "All":
                pledge_file_path = InitDatabase.getInstance().initilize_pledgeitem_database(trustText,
            ...
            if total_records > 0:
                ...
                for row_index in range(0, total_records):
                    ...
                    # print("dateFromMon_DepositSheet :", dateFromTransactionSheet, "fromDate :", fromDate, " toDate:", toDate)
                    print("Selected Pledge item filter is :", categoryText.get())
                    if sheet_obj.cell(row=row_index + 2, column=13).value == trustStatusText.get() and \
                            sheet_obj.cell(row=row_index + 2, column=2).value == memberId_Text.get():
                        for column_index in range(1, 14):
                            text_value = str(sheet_obj.cell(row=row_index + 2, column=column_index).value)
                            sheet_sorted_obj.cell(row=sort_sheet_index, column=column_index).font = Font(
                                size=8,
            ...
            text_error = "Invalid Member Id !!! "
            infoLabel.configure(text=text_error, fg='red')

    def viewPDF(self, template_statement,
                destination_file, starting_index):
        os.startfile(destination_file)
        self.clearStockInfoTemplateSheet(template_statement, starting_index)

    def clearTemplateSheet(self, template_statement, starting_index):
        # executed only if
        wb_template = openpyxl.load_workbook(template_statement)
        template_sheet = wb_template.active
        print("starting_index :", starting_index)
        for rows in range(5, starting_index + 1):
            for columns in range(1, 9):
